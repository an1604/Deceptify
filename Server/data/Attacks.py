...
class AttackFactory:
    ...
    @staticmethod
    def create_attack(attack_type: str, campaign_name: str, mimic_profile: Profile, target: Profile, description: str, camp_id: int, recordings=None, transcript=None):
        """
        ...
        """
        allowed_attack_types = ['Voice', 'Video']  # Define your allowed attack types here
        if attack_type == 'Voice':
            return VoiceAttack(campaign_name, mimic_profile, target, description, camp_id, recordings, transcript)
        if attack_type == 'Video':
            return VideoAttack(campaign_name, mimic_profile, target, description, camp_id, recordings, transcript)
        else:
            raise ValueError(f"Invalid attack_type. Allowed values are {allowed_attack_types} i got {attack_type}")
...
class VoiceAttack(Attack):
    ...
    def from_dict(data):
        ...
        return VoiceAttack(campaign_name, mimic_profile, target, description, camp_id,
    ...
    def __hash__(self):
        return hash((self.campaign_name, self.mimic_profile, self.target, self.description, self.id,self.recordings, self.transcript))
...
class VideoAttack(Attack):
    ...
    def __init__(self, campaign_name, mimic_profile, target, description, camp_id, video_recordings, transcript=None):
        super().__init__(campaign_name, mimic_profile, target, description, camp_id)
    ...
    def from_dict(data):
        ...
        recordings = data['recordings']
        transcript = data['transcript']
        return VideoAttack(campaign_name, mimic_profile, target, description, camp_id,
                            recordings, transcript)
    ...
    def __hash__(self):
        return hash((self.campaign_name, self.mimic_profile, self.target, self.description, self.id,self.video_recordings, self.transcript))
